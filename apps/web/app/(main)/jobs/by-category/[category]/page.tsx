...
export async function generateStaticParams() {
  return CATEGORIES.map(({ code }) => ({ category: code }))
}
...
export async function generateMetadata({
  ...
  return generateFilterPageMetadata('by-category', category, count || 0)
}

export default async function JobsByCategoryPage({ params }: PageProps) {
  const { category } = await params

  // Validate category against CATEGORIES
  const categoryData = CATEGORIES.find((c) => c.code === category)
  if (!categoryData) {
    notFound()
  }
  ...
  // Query jobs filtered by category
  ...
  const { title, titleKo } = getFilterPageTitle('by-category', category)
  const description = getFilterPageDescription('by-category', category)
  const faqs = getFilterPageFAQs('by-category', category)
  const crossLinks = getFilterCrossLinks('by-category', category)
