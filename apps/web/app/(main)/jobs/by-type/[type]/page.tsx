...
export async function generateMetadata({
  ...
  // Validate type
  const jobType = JOB_TYPES.find((t) => t.code === type)
  if (!jobType) {
    return {
  ...
  return generateFilterPageMetadata('by-type', type, count || 0)
}

export default async function JobsByTypePage({ params }: PageProps) {
  const { type } = await params

  // Validate type against JOB_TYPES
  const jobType = JOB_TYPES.find((t) => t.code === type)
  if (!jobType) {
    notFound()
  }
  ...
  const { title, titleKo } = getFilterPageTitle('by-type', type)
  const description = getFilterPageDescription('by-type', type)
  const faqs = getFilterPageFAQs('by-type', type)
  const crossLinks = getFilterCrossLinks('by-type', type)
