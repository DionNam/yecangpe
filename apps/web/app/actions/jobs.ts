...
export async function createJobPost(formData: FormData) {
  ...
  const { data: newPost, error: insertError } = await (supabase as any)
    ...
    .insert({
      ...
      salary_negotiable: formData.get('salary_negotiable') === 'true',
      salary_min: formData.get('salary_negotiable') === 'true' ? null : (result.data.salary_min ?? null),
      salary_max: formData.get('salary_negotiable') === 'true' ? null : (result.data.salary_max ?? null),
      salary_currency: result.data.salary_currency,
      salary_period: formData.get('salary_negotiable') === 'true' ? null : (result.data.salary_period || null),
